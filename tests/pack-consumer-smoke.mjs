...
try {
  ...
  // The version a package reports has to match the version it shipped as. It is
  // checked from an installed tarball because the failure is one of packaging
  // layout: reading the manifest works in the repo and can still break once the
  // entry moves into dist/.
  write(join(consumer, 'packed-version-smoke.mjs'), `import { readFileSync } from 'node:fs';\nimport { VERSION as cli } from '@openpresent/cli';\nimport { VERSION as mcp } from '@openpresent/mcp';\nconst shipped = (name) => JSON.parse(readFileSync(\`node_modules/\${name}/package.json\`, 'utf8')).version;\nfor (const [name, reported] of [['@openpresent/cli', cli], ['@openpresent/mcp', mcp]]) {\n  const declared = shipped(name);\n  if (reported !== declared) throw new Error(\`\${name} reports version \${reported} but shipped \${declared}.\`);\n}\nconsole.log(JSON.stringify({ versionsAgree: cli }));\n`);
